...
All coordinator communication is routed through `@p2payments/tor`, which this module installs automatically. No direct Tor/SOCKS dependency is required in the consuming app.
...
| `NUXT_TOR_PROXY_SECRET` | yes | — | Shared secret for the `@p2payments/tor` proxy |
...
`@p2payments/tor` is installed automatically — no need to add it separately.
